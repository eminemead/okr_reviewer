...
def generate_okr_report(okr_limit=2):
    """
    Generate OKR report using manager fellow_ad_accounts from DuckDB.
    ...
    # Use okr_ingest_manager.py (manager fellow_ad_accounts method)
    script_name = "okr_ingest_manager"
    print("Using manager fellow_ad_accounts method (okr_ingest_manager)")
    ...
    try:
        ...
        expected_filename = os.path.join(reports_dir, f"okr_report_manager_{timestamp}.md")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Generate OKR report with timestamped filename for managers")
