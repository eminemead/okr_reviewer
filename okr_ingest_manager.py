...
def main(args):
    ...
    try:
        # Query the new source table with filters for managers, ensuring fellow_ad_account is not null, and limit to 10k rows
        rows = conn.execute("""
            ...
            WHERE 
                ...
                AND fellow_workday_cn_title_code IN ('CUSTOM_ORGANIZATION-3-4823', 'CUSTOM_ORGANIZATION-3-6350', '100573', 'CUSTOM_ORGANIZATION-3-4833', 'CUSTOM_ORGANIZATION-3-7100', 'CUSTOM_ORGANIZATION-3-7099', 'CUSTOM_ORGANIZATION-3-6265', '100577', 'CUSTOM_ORGANIZATION-9-6521')
        """).fetchall()
    ...
    report_filename = os.path.join(output_dir, f"okr_report_manager_{timestamp}.md")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Fetch OKRs for all manager fellow_ad_accounts in DuckDB and write to markdown report.")
